...
class NeuroMet():
    ...
    def __init__(self, sublist, temp_dir, w_dir, omp_nthreads, raw_data_dir, overwrite, qdec_subjects, qdec_subfolder):
    ...
    def generate_qdec_file(self):
        ...
        if self.qdec_subjects == []:
            fs_dirs = glob.glob(self.w_dir + '/NeuroMet2[0-9][0-9][0-9]/NeuroMet2[0-9][0-9][0-9].freesurfer')
        else:
            fs_dirs = [(self.w_dir + '/NeuroMet2{0}/NeuroMet2{0}.freesurfer'.format(f)) for f in self.qdec_subjects]
        subdirs_list = []
        for d in fs_dirs:
            subdirs_list.append('/'.join(d.split('/')[-2:]))
        qdec_filename = os.path.join(qdec_dir, 'Freesurfer_qdec_{prefix}.dat'.format(prefix=self.subject_prefix))
